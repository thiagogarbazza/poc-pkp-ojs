...
echo "[backup] Deleting all expired '$TYPE' backup created more than $EXPIRED_DAYS days ago."
find /data/backups -maxdepth 1 -mtime +$EXPIRED_DAYS -name "*$TYPE*" \
  -exec echo "Remove backup file: {}" \; \
  -exec rm -rf {} \;

echo "[backup] Cleaning and creating temporary directory for backup creation"
TMP_DIR=/tmp/backup-poc-pkp-ojs/$BACKUP_FILE_NAME
rm -rf $TMP_DIR
mkdir -p $TMP_DIR

echo "[backup] Creating backup file of $OJS_FILE_DIR"
tar -C $OJS_FILE_DIR -czf $TMP_DIR/ojs-files.tar.gz --exclude="*.bkp" --exclude="*.log" --exclude="*.mmdb" .

echo "[backup] Creating backup file of $OJS_PUBLIC_DIR"
tar -C $OJS_PUBLIC_DIR -czf $TMP_DIR/ojs-public.tar.gz --exclude="*.bkp" --exclude="*.log" --exclude="*.mmdb" .

echo "[backup] Creating database backup file"
...
echo "[backup] Creating general backup file and moving to directory $BACKUP_DIR/$BACKUP_FILE_NAME.tar.gz"
rm -rf $BACKUP_DIR/$BACKUP_FILE_NAME.tar.gz
tar -C $TMP_DIR -czf $BACKUP_DIR/$BACKUP_FILE_NAME.tar.gz .

echo "[backup] Cleaning temporary files"
rm -rf $TMP_DIR

echo "[backup] All exists backup files:"
ls -la $BACKUP_DIR
